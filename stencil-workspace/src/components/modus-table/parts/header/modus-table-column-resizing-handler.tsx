 ...
 */
function columnResizingOnKeyDown(
  event: KeyboardEvent,
  table: Table<unknown>,
  ...
  if (eventKey === EnterKey || eventKey === SpaceKey || eventKey === TabKey) {
    ...
    const columnSize =
      table.options.state.columnSizing[column.id] ||
      column.columnDef.size ||
      150; // Having 150 since the default size column is 150 if size is not mentioned.
...
export const ModusTableColumnResizingHandler: FunctionalComponent<
  ModusTableColumnResizingHandlerProps
> = ({ table, header }) => {
  return (
    <div
