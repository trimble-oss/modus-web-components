...
export class ModusTreeView {
  ...
  handleKeyDown(event: KeyboardEvent): void {
    ...
    // When Tabbing is disabled:
    // Allow up/down arrows keys for navigation
    // Allow tab key for reseting the focus item
    const noTabbingAllowedKey =
      key === 'ARROWUP' || key === 'ARROWDOWN' || key === 'TAB';

    // Either focusItem should not be empty or tabbing is disabled and arrows pressed
    if (
      event.altKey ||
      (!this.focusItem && !(noTabbingAllowedKey && this.disableTabbing))
    ) {
      return;
    }
    switch (key) {
      ...
      case 'ARROWDOWN':
        ...
        let nextItem: string;
        if (this.disableTabbing && !this.focusItem) {
          nextItem = this.getFirstItem();
        } else nextItem = this.getNextNavigableItem(this.focusItem);
      ...
      case 'ARROWUP':
        ...
        } else prevItem = this.getPrevNavigableItem(this.focusItem);
