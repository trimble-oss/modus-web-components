...
interface ModusDataTableColumnsVisibilityOptions {
  title: string;
  requiredColumns?: string[];
}

interface ModusDataTablePanelOptions {
  columnsVisibility?: ModusDataTableColumnsVisibilityOptions;
}
```
...
| Column Visibility | The meatball icon buttom will get focus by using the `Tab` key. By pressing `Enter or Spacebar`, a dropdown will be opened; press the `Escape` key to close the dropdown. Using `Tab or Shift+Tab`, column checkboxes can get focus to change state by pressing the `Enter or Spacebar`.|
