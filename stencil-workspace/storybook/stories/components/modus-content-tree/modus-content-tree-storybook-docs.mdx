...
| `disableTabbing`         | `disable-tabbing`          | (optional) Disable navigation through tree using `tab` key, use `up/down` arrow keys for navigation. | `boolean`                              | `undefined`  |
...
Every item and the elements such as drag icon, expand & collapse icon and the checkbox are accessible through keyboard. `Tab` key moves focus to the next focusable element and `Shift + Tab` moves focus to the previous focusable element provided `disableTabbing` is not set `true`.
