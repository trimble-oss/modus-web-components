...
- This component utilizes the slot element, allowing you to render custom expand icon, collapse icon, drag icon, item icon. The slot names are `expandIcon`, `collapseIcon`, `dragIcon`, `itemIcon`, also a slot to add a side content to the right in cases you want a label to the left and another content to the right.
...
<Story id="components-content-tree--with-side-content" />

```html
<modus-tree-view
    style="width:400px;"
    checkbox-selection="false"
    disable-tabbing="false"
    multi-checkbox-selection="false"
    multi-selection="false"
    size="starndard">
      <modus-tree-view-item node-id="1" label="Inbox" class="hydrated">
...
Every item and the elements such as drag icon, expand & collapse icon and the checkbox are accessible through keyboard. `Tab` key moves focus to the next focusable element and `Shift + Tab` moves focus to the previous focusable element. If `disableTabbing` is set `true`, tabbing inside the tree isn't allowed. Please refer to the next section for more details on keyboard interaction.
...
This component utilizes the slot element for the following:

| Slot             | Description                                 |
| ---------------- | --------------------------------------------|
| `label`          | Slot for label text.                        |
| `groupLeft`      | Slot for custom content to the left.        |
| `groupRight`     | Slot for custom content to the right.       |
| `expandIcon`     | Slot for render custom expand icon.         |
| `collapseIcon`   | Slot for render custom collapse icon.       |
| `dragIcon`       | Slot for render custom drag icon.           |
| `itemIcon`       | Slot for render custom item icon.           |
