  ...
  <modus-tree-view>
    ...
    <modus-tree-view-item node-Id="6" label="Archived">
      <modus-tree-view-item node-Id="7" label="Folder1">
        ...
        <modus-tree-view-item node-Id="9" label="File3"></modus-tree-view-item>
      </modus-tree-view-item>
    </modus-tree-view-item>
  </modus-tree-view>
</div>
<script>
  function actionBarScript() {
    ...
    let items = new Map();
    root.querySelectorAll('modus-tree-view-item').forEach((element) => {
      items.set(element.nodeId, element.label);
    });
    ...
    addButton.addEventListener('buttonClick', () => {
      ...
      if (nodeId) {
        ...
        newNode.draggableItem = insertBeforeElement.draggableItem;
        newNode.droppableItem = insertBeforeElement.droppableItem;
    ...
    copyButton.addEventListener('buttonClick', () => {
      const nodeId = root.childElementCount + 1;
      const selectedItems = root.selectedItems;
      const selectedItemId =
        selectedItems && selectedItems.length > 0 ? selectedItems[0] : null;
      const selectedItemElement = querySelect(selectedItemId);
    ...
    dragButton.addEventListener('buttonClick', () => {
      items.forEach((val, key) => {
        ...
        element.draggableItem = !element.draggableItem;
...
| Enter                 | Using `Tab` key if the focus is set on an item, pressing `Enter` key will select it and if the focus is on an element like checkbox, collapse/expand icon, pressing `Enter` key will perform operations only on that element |
...
| draggableItem | Allows the item to be dragged across the tree                                                                          | boolean |         |               |                                              |
| droppableItem | Allows the item to be a drop zone so other tree items can be dropped above it                                          | boolean |         |               |                                              |
| editable      | Changes the label text into a input text box for editing (Note: applicable only when the value is set by prop `label`) | string  |         |               | <span class="material-icons">&#10004;</span> |
