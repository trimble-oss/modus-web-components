...
| selectionChange | Fired when the item's selection state changes by any means, such as mouse or keyboard interactions                                    | `CustomEvent<{isSelected: boolean, itemId: number}>` |
