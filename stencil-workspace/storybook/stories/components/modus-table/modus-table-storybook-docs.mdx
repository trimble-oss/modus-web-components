...
Manual Pagination allows users to handle pagination in server-side mode.

- The Modus Table utilizes the `modus-pagination` component for navigating between pages. Based on the properties initially set, pagination can be managed from the server side.
...
- The `pageCount` takes a number to specify the total number of records, in order to display the corresponding number of pages in the table.
