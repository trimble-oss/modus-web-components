...
User can add up to 4 actions to the beggining of the rows with `rowActions`.

- Set `_excludedActions` with an array of action ids that should not be displayed for specific rows in `data`.

<Story id="components-table--row-actions" height={'350px'} />

### Overflow Menu

User can add an overflow menu to the beggining of the rows with `overflowMenuActions`.

- Set `_excludedActions` with an array of action ids that should not be displayed for specific rows in `data`.

<Story id="components-table--overflow-menu" height={'350px'} />
