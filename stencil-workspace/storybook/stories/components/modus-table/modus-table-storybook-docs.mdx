...
```
### Inline Editing - Dynamic Filtering

The dynamic filtering feature in the modus-table component leverages an existing autocomplete component to enhance user interaction. This feature is integrated through the cellEditorArgs property of the table’s columns. When a cell with an autocomplete editor is activated, the filterOptions function is invoked with the user’s input value. This function dynamically filters the list of available options based on the input, providing real-time suggestions that are relevant to the user's search criteria
...
```javascript
  ...
  table.columns = [
    {
      ...
      cellEditorArgs: {
        ...
        loading: filterOptionsLoading, // Pass the loading state
  ...
  function filterDynamicOptions(value) {
    return new Promise((resolve) => {
      // Set loading state to true and update the table
      ...
      setTimeout(() => {
        ...
        // Reset loading state and update the table
  ...
  /**
    * Helper function to update the table columns and reflect the current loading state
    */
  function updateTableColumns() {
    // Update the 'First Name' column to reflect the loading state
    table.columns = table.columns.map((col) => {
      if (col.id === 'first-name') {
        return {
          ...col,
          cellEditorArgs: {
            ...col.cellEditorArgs,
            loading: filterOptionsLoading, // Update the loading state in the arguments
          },
        };
      }
      return col;
