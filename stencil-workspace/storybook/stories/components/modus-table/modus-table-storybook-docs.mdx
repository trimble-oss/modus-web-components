...
- Set the action's `isVisible: (row) => boolean` to false to hide the action for specific rows.

<Story id="components-table--row-actions" height={'350px'} />

### Overflow Menu

A maximum of 4 icons are shown in the cell. When this limit is exceeded, the remaining actions are shown in an overflow menu.
...
| `rowActions`           | --                        | (Optional) Actions that can be performed on each row. A maximum of 4 icons will be shown, remaining actions will be at overflow menu. | `ModusTableRowAction[]`                | `[]`                                                      |
