...
Users can display a checkbox in a table column.

- Pass the data in `ModusTableCellCheckbox` format and specify `ModusTableCellCheckbox.checkbox` as the column type.
- As a checkbox is an object, the default sorting is ineffective. We must pass `sortingFn:'sortForCheckbox'` along with the column information in order to sort a column with the `ModusTableCellCheckbox` datatype or if '\_type' is used to override the datatype.
