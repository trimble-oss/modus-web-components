...
There are two options under `rowSelectionOptions`: `multiple` and `subRowSelection`.

  - To enable multiple row selection, set `multiple` to `true`.
  - Sub-rows will also be selected when the value of the'subRowSelection' is set to 'true'.
...
| `rowSelectionOptions`     | `checkbox-selection-options`| (Optional) To control multiple row selection.                                                               | `boolean`                | `false`      |
...
| `rowSelectionChange` | Event details contains the row(s) selected   | `[{}]`                                                   |
### Slot
