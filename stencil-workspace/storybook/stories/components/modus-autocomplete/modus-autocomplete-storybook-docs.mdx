...
<Story id="user-inputs-autocomplete--with-chips" />
...
<div style="width: 500px">
  ...
  <!-- With ModusAutocompleteOption model (values set in <script />) where chips are added -->
  <modus-autocomplete id="with-chips" label="Autocomplete using option model" add-chip placeholder="Search..."></modus-autocomplete>
...
<script>
  ...
  document.querySelector('#with-option').options = [
    ...
    document.querySelector('#with-chips').options = [
    { id: '0', value: 'Apple' },
    { id: '1', value: 'Banana' },
    { id: '2', value: 'Orange' }
  ];
...
|`addChip`                    | `add-chip`                      | Whether to add a chip when an option is selected. | `boolean`                            | `false`                                       |
### Events
