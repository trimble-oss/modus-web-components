...
[Modus Floating Navbar](https://modus.trimble.com/components/navbar/) web components are the header of all Trimble applications. They are referenced using the `<modus-floating-navbar>` custom HTML element.
...
- If `buttons` are provided, Floating navbar creates the icon buttons dynamically
...
<Story id="components-floating-navbar--default" />
...
<script>
  const element = document.querySelector('modus-floating-navbar');
  element.apps = [
    {
...
<Story id="components-floating-navbar--failed-avatar" />
...
Use the `variant` prop to choose a blue background floating navbar.

<Story id="components-floating-navbar--blue-navbar" />
...
Use `slot="left"` and `slot="right"` to add your own components on the floating navbar.

<Story id="components-floating-navbar--used-slots" />
...
```ts
interface ModusFloatingNavbarApp {
  description?: string;
  logoUrl: string;
  name: string;
  url: string;
...
interface ModusFloatingNavbarProfileMenuLink {
  display: string;
  id: string;
}

interface ModusFloatingNavbarLogo {
  url: string;
  height?: string;
}

interface ModusFloatingNavbarLogoOptions {
  primary?: ModusFloatingNavbarLogo;
  secondary?: ModusFloatingNavbarLogo;
}

interface ModusFloatingNavbarTooltip {
    text: string;
    ariaLabel?: string;
}

interface ModusFloatingProfileMenuOptions {
  avatarUrl?: string;
  email?: string;
  initials?: string;
...
interface ModusFloatingNavbarButton {
  id: string;
  icon: string;
  orderIndex: number;
  hideMenu?: boolean;
...
| `buttons`              | --                       | (optional) The buttons to render in the floating navbar.                                                                                                                                         | `ModusNavbarButton[]`     | `undefined` |
...
| `reverse`              | `reverse`                | (optional) Whether to display the floating navbar items in reverse order.                                                                                                                        | `boolean`                 | `undefined` |
...
| `showShadow`           | `show-shadow`            | (optional) Whether to show a shadow under the floating navbar.                                                                                                                                   | `boolean`                 | `undefined` |
| `variant`              | `variant`                | (optional) Color variants for floating navbar.                                                                                                                                                   | `"blue" \| "default"`     | `'default'` |
