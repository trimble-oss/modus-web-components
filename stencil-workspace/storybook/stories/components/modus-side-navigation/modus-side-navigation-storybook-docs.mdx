...
- There are two ways to add a Modus Side Navigation. One way is by using the components `modus-side-navigation` for root and `modus-side-navigation-item` for menu items and another way is to pass an object array for the prop `data`. Only one method can be used at a time.
...
Every menu item is accessible through keyboard. `Tab` moves focus to the next focusable element and `Down Arrow` key moves focus to the next menu item. `Shift + Tab` moves focus to the previous focusable element and `Up Arrow` moves focus to the previous menu item.
