...
Angular has built-in support for web components and custom elements. Working with web components in Angular is straightfoward, they work with the same template API we are familiar with.
...
A strong suggestion we have is to wrap Modus Web Components in corresponding Angular components within your application.
...
@Component({
  ...
  template: `
    <modus-button [buttonStyle]="buttonStyle" [color]="color" [disabled]="disabled" [size]="size" (buttonClick)="onButtonClick.emit()">
...
export class ButtonComponent {
  ...
  @Input() color: "danger" | "default" | "primary" | "secondary" | "warning" = "default";
...
```html
<select-component [formControl]="$any(form).controls['select1']" [label]="'Select Form Demo'" [options]="options" [optionsDisplayProp]="'display'"> </select-component>
```
